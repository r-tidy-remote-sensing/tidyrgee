...
modis_yr_mo <- modis_ic_tidy |> 
  tidyrgee::group_by(year,month) |> 
  summarise(stat = "mean")
```
...
modis_monthly_baseline_sd <- modis_ic_tidy |> 
  tidyrgee::select("NDVI") |> 
  tidyrgee::filter(year %in% 2000:2015) |> 
  tidyrgee::group_by(month) |> 
  tidyrgee::summarise(stat="sd")
